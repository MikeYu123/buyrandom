class AuctionsController < ApplicationController
  ...
  def show
    @auction = Auction.includes(:product, bids: :source).find(params[:id])
  end

  def new
    @product = Product.find(params[:product_id])
    @auction = InitAuctionService.new.build_auction(@product)
  end

  def create
    @product = Product.find(params[:auction][:product_id])
    @auction = InitAuctionService.new.create_auction(@product)
